...
assertion^[Statistics is designed for uncertainty, so no offense taken at your curiosity.], 
...
Those of you with some background in hypothesis testing already will no doubt notice that we already use alpha as a symbol for the significance level. This is not an accident: **the level of a test is the probability that such a test makes a type I error, and for any choice of $\alpha$, we use tests with a level up to $ \alpha $ **. 

In other words, we figure out which test to use based on our tolerance for convicting an innocent null. 
